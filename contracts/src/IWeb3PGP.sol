...
interface IWeb3PGP {
     ...
     * 
     * @param fingerprint The fingerprint of the public key for which the challenge is issued.
     ...
     * 
     * @param fingerprint The fingerprint of the public key for which the ownership is proved.
     * @param challenge The challenge that was signed to prove ownership of the key.
     ...
     * @param challenge The challenge to be signed to prove ownership of the key.
     */
    function challengeOwnership(bytes32 fingerprint, bytes32 challenge) external payable;

    /**
     * @notice Submits the signed challenge to prove ownership of the public key with the given fingerprint.
     * @dev The signature must be a binary OpenPGP signature of the challenge, eg. the hash, so it is publicly verifiable.
     * @param fingerprint The fingerprint of the public key for which ownership is being proved.
     * @param challenge The challenge that was signed to prove ownership of the key.
     * @param signature The binary OpenPGP signature of the challenge using the private key corresponding to the public key.
     */
    function proveOwnership(bytes32 fingerprint, bytes32 challenge, bytes calldata signature) external payable;

    /**
     * @notice Publishes a key certification attesting that an issuer certifies the identity linked to a key (Web of Trust).
     * @dev The keyCertificate must be a binary OpenPGP key certificate containing the public key, the user ID packet, the self-certification and
     * the third-party certification signature from the issuer in case this is not a self-certification.
     * @param fingerprint The fingerprint of the public key being certified.
     * @param issuerFingerprint The fingerprint of the issuer's public key. In case this is a self-certification, both fingerprints are identical.
     * @param keyCertificate The binary OpenPGP message containing the key certification signature.
     */
    function certifyKey(bytes32 fingerprint, bytes32 issuerFingerprint, bytes calldata keyCertificate) external payable;

    /**
     * @notice Revokes a key certification previously published for a given public key by an issuer.
     * @dev The revocationSignature must be a binary OpenPGP signature revoking the key certification previously published.
     * @param fingerprint The fingerprint of the public key whose certification is being revoked.
     * @param issuerFingerprint The fingerprint of the issuer's public key that issued the certification.
     * @param revocationSignature The binary OpenPGP signature revoking the key certification.
     */
    function revokeCertification(bytes32 fingerprint, bytes32 issuerFingerprint, bytes calldata revocationSignature) external payable;
    ...
    /**
     * @notice Returns the block numbers which contains certifications issued for a given public key.
     * @param fingerprint The fingerprint of the public key to retrieve certifications for.
     * @param start The starting index for the certification list.
     * @param limit The maximum number of certifications to return.
     */
    function listCertifications(bytes32 fingerprint, uint256 start, uint256 limit) external view returns (uint256[] memory);

    /**
     * @notice Returns the block numbers which contains certification revocations issued for a given public key.
     * @param fingerprint The fingerprint of the public key to retrieve certification revocations for.
     * @param start The starting index for the certification revocation list.
     * @param limit The maximum number of certification revocations to return.
     */
    function listCertificationRevocations(bytes32 fingerprint, uint256 start, uint256 limit) external view returns (uint256[] memory);
}
