...
export function validateYamlStructure(data: unknown): void {
  ...
  if (!config.web3sign) {
    throw new ConfigError('Missing required field: web3sign');
  }
  ...
  // Validate web3sign section
  const web3sign = config.web3sign;
  if (typeof web3sign !== 'object' || web3sign === null) {
    throw new ConfigError('web3sign must be an object');
  }
  if (!('contract' in web3sign)) {
    throw new ConfigError('Missing required field: web3sign.contract');
  }
