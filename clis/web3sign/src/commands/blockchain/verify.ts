...
import { to0x, Web3SignService } from '@jibidieuw/dexes';
import { toBytes, toHex } from 'viem';
import { Config } from 'openpgp';

export interface VerifyCommandDeps {
  logger: Logger;
  web3signService: Web3SignService; 
}
...
export function createVerifyCommand(deps: VerifyCommandDeps): Command {
  const { logger, web3signService } = deps;
    ...
    .action(async (input: string | undefined, options: { id?: string; all?: boolean; doc?: boolean; hash?: boolean }) => {
      try {
        ...
        if (options.id) {
            ...
            const timestamp = await web3signService.verifyTimestamp(BigInt(options.id));
          ...
          const ids = await web3signService.findTimestampsByHash(toBytes(documentHash));
          if (ids.length === 0) {
