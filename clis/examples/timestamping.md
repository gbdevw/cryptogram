...
This guide will show you how to use web3doc CLI to timestamp and verify a document.
...
The second step is to compute the keccak256 hash of the document using the web3doc CLI 'keccak256' command. The timestamp requires the signature to be made over the bytes of the keccak256 hash of the document.

```bash
HASH_DATA=hash.data
web3doc keccak256 -p $FILE_TO_TIMESTAMP > $HASH_DATA
...
Use the timestamp command of the web3doc CLI to publish the detached signature and the hash of the document on-chain.

```bash
export DEXES_WALLET_PRIVATE_KEY='0x...' # pragma: allowlist secret
web3doc timestamp -e $KEYFP -H $DOCHASH -s $SIGFILE
```
...
```bash
export DEXES_WALLET_PRIVATE_KEY='0x...' # pragma: allowlist secret
cat $SIGFILE | web3doc timestamp -e $KEYFP -H $DOCHASH
```
...
web3doc verify --id $TIMESTAMP_ID --doc $FILE_TO_TIMESTAMP
```
...
```bash
TIMESTAMP_ID=<your_timestamp_id>
cat $WRONG_FILE | web3doc verify --id $TIMESTAMP_ID --doc
```
...
```bash
web3doc verify --id $TIMESTAMP_ID --hash $DOCHASH
```

7. Play with verify command

A document can be timestamped many times. Or, the user might find a document and a USB stick and want to find records of it in Web3DOC but you do not know any timestamp ID which might match the document. In such case, you can provide the --all flag as instead of the --id flag. The CLI will use the provided hash or will hash the provided document and search all timestamps which have the same hash within their data. The CLI will download and verify each timestamp and only emit valid signatures on stdout.

```bash
web3doc verify --all --doc $FILE_TO_TIMESTAMP
```