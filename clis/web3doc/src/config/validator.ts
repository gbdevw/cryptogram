...
export function validateYamlStructure(data: unknown): void {
  ...
  const web3doc = config.web3doc;
  if (typeof web3doc !== 'object' || web3doc === null) {
    throw new ConfigError('web3doc must be an object');
  }
  if (!('contract' in web3doc)) {
    throw new ConfigError('Missing required field: web3doc.contract');
  }
